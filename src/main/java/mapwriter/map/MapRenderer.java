...
public class MapRenderer {
	...
	private ResourceLocation backgroundTexture = this.createIcon("mapwriter", "textures/map/background.png");
	private ResourceLocation roundMapTexture = this.createIcon("mapwriter", "textures/map/border_round.png");
	private ResourceLocation squareMapTexture = this.createIcon("mapwriter", "textures/map/border_square.png");
	private ResourceLocation playerArrowTexture = this.createIcon("mapwriter", "textures/map/arrow_player.png");
	private ResourceLocation northArrowTexture = this.createIcon("mapwriter", "textures/map/arrow_north.png");

	ResourceLocation createIcon(String nameSpace, String location) {
	...
	private void drawMapTexture() {
		...
		if ((this.mapView.getUndergroundMode()) && (regionZoomLevel == 0)) {
			...
			if (this.mw.backgroundTextureMode > 0) {
				...
				this.mw.mc.getTextureManager().bindTexture(this.backgroundTexture);
				Render.setColourWithAlphaPercent(0xffffff, this.mapMode.alphaPercent);
				Render.drawTexturedRect(
						this.mapMode.x, this.mapMode.y, this.mapMode.w, this.mapMode.h,
						bu1, bv1, bu2, bv2
	...
	private void drawBorder() {
		if (this.mapMode.circular) {
			this.mw.mc.getTextureManager().bindTexture(this.roundMapTexture);
		} else {
			this.mw.mc.getTextureManager().bindTexture(this.squareMapTexture);
		}
	...
	private void drawPlayerArrow() {
		...
		Render.setColour(0xffffffff);
		this.mw.mc.getTextureManager().bindTexture(this.playerArrowTexture);
		Render.drawTexturedRect(
	...
	private void drawIcons() {
		...
		if (this.mapMode.rotate) {
			...
			double arrowSize = this.mapMode.playerArrowSize;
			Render.setColour(0xffffffff);
			this.mw.mc.getTextureManager().bindTexture(this.northArrowTexture);
			Render.drawTexturedRect(
